...
func (t *T) fromMap(m map[string]interface{}) error {
	...
	if v,ok := m["messages"]; ok {
		for _,x := range v.([]interface{}) {
			...
			if err := m0.Kind.fromString(y["kind"].(string)); err != nil { return err }
	...
	if v,ok := m["pprof"]; ok {
		...
		for _,x := range v.([]interface{}) {
			y := x.(map[string]interface{})
			p0 := &Report{}
			if err := p0.Unit.fromString(y["unit"].(string)); err != nil { return err }
			if err := p0.Options.fromString(y["options"].(string)); err != nil { return err }
